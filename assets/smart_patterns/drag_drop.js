function Drag_drop() {
  ...
  this.bindAddFormEvents = function () {
    ...
    enableCkeditor("id1");
    enableCkeditor("id2"); // for (var i = 0; i < this.settings.stepsOption.length; i++) {
    //     enableCkeditor("id" + i);
    // }

    var that = this;$('#btnColor').spectrum(getColorPickerForSpectrum(that.settings.submitColor));
    ...
    $("#dragdrop_widget_form").on("submit", function (e) {
      ...
      that.settings.submit = $(e.target).find("[name='submit']:checked").val();
...
Drag_drop.prototype = Object.create(MainWidget.prototype);