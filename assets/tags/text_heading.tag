<Text_heading>
  ...
  <div class="animation_this word_count">
    <div  contenteditable="true" id={editor_id} class='text-widget' ref="main" onclick={clearText} onfocusout={focusOutText} onpaste={pasteText} onkeyup = {keyUpText}>
      {widgetText}
    </div>
 </div>
   ...
   this.clearText = function(event) {
    ...
    this.focusOutText = function(event) {
      ...
      if ($("#".concat(this.editor_id)).text().trim() === "" && $("#".concat(this.editor_id, " table")).length === 0) {
        $("#".concat(this.editor_id)).html("<div tabindex='0' class='default_text'>" + opts.settings.defaultText + "</div>");
...
this.on("mount", function () {
  ...
  if (opts.settings.text == opts.settings.defaultText && opts.settings.heading == opts.settings.defaultHeading) {
    this.refs.main.innerHTML = "<div tabindex=\"0\" class=\"default_heading\"><h1>".concat(opts.settings.heading, "</h1></div><div class='default_text' tabindex='0'>").concat(opts.settings.text, "</div>");
  } else {
