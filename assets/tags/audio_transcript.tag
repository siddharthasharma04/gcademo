<audio_transcript>
    ...
    <div class="animation_this word_count">
        <div class="transcript_text" tabindex="0">Audio Transcript</div>
        <div contenteditable="true" tabindex="0" id={editor_id} class='transcript-widget' ref="main" onclick={clearText} onfocusout={focusOutText}>
            {widgetText}
        </div>
    </div>
...
this.focusOutText = function(event) {
  var el = $(event.target);
  if ($("#".concat(this.editor_id)).text().trim() === "" && $("#".concat(this.editor_id, " table")).length === 0) $("#".concat(this.editor_id)).html(opts.settings.defaultText);
}
